...
struct RegisterRequest {
    1: string username (api.query="username", api.vd="len($) > 0") // 用户期望的用户名
    2: string password (api.query="password", api.vd="len($) > 0") // 用户密码
}
...
// 用户登录请求结构体
struct LoginRequest {
    1: string username (api.query="username", api.vd="len($) > 0") // 用户登录的用户名
    2: string password (api.query="password", api.vd="len($) > 0") // 用户登录的密码
}
